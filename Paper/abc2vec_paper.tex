...
To understand the contribution of each pre-training objective, we conduct a systematic ablation study by training six model variants with different objective combinations. All variants use identical architecture (6 layers, 256-dimensional, 8 attention heads) and training hyperparameters, differing only in which loss functions are active during training. This controlled setup allows us to isolate the contribution of each objective and assess whether objectives exhibit synergistic effects when combined.

\subsection{Experimental Setup}

We train the following six variants:
...
\begin{enumerate}
    ...
    \item \textbf{MMM+TI+SCL} (Full model): All three objectives ($\lambda_{\text{MMM}}=1.0$, $\lambda_{\text{TI}}=0.5$, $\lambda_{\text{SCL}}=0.5$)
\end{enumerate}
...
All six model variants achieve nearly identical performance on both tune type and mode classification tasks. Tune type accuracy ranges narrowly from 46.0\% to 46.9\% and mode accuracy remains consistently at 80.7\% across all variants. The full model (MMM+TI+SCL) achieves 46.7\% tune type accuracy and 80.7\% mode accuracy, statistically indistinguishable from single-objective models. This suggests that the pre-training objectives primarily shape the embedding space structure rather than directly improving downstream classification accuracy. The detailed results are shown in Table~\ref{tab:ablation_classification}.

\begin{table}[H]
\centering
\caption{Ablation study: Tune type and mode classification performance (5-fold CV). All models show comparable accuracy, indicating that objectives affect embedding structure more than classification performance.}
...
MMM+TI+SCL (Full) & 0.467 & 0.007 & 0.807 & 0.001 \\
\bottomrule
\end{tabular}
\end{table}
...
Linear probing uses a single-layer ridge classifier trained on frozen embeddings, providing a stricter test of embedding quality than full fine-tuning. Across four properties—tune type, mode, key root, and tune length—all variants perform comparably, with tune type at 46.1-46.8\%, mode at 80.6-80.7\%, key root at 31.4-32.2\%, and tune length at 93.2-93.8\%. These results mirror the classification findings and are detailed in Table~\ref{tab:ablation_linear_probing}.

\begin{table}[H]
\centering
\caption{Ablation study: Linear probing accuracy on multiple properties. Consistent performance across variants suggests that all objectives induce comparable linear separability.}
...
MMM+TI+SCL (Full) & 0.468 & 0.807 & 0.314 & 0.934 \\
\bottomrule
\end{tabular}
\end{table}
...
Unsupervised clustering metrics reveal that all variants produce embeddings with some internal structure, though clusters are not sharply separated. Silhouette scores range from 0.491 to 0.575, with the full model (MMM+TI+SCL) achieving the highest tune type silhouette score (0.575), suggesting that combining objectives may yield slightly more structured embeddings compared to single-objective models, though the difference is modest (+8.4\% vs. MMM-only). These results are summarized in Table~\ref{tab:ablation_clustering}.

\begin{table}[H]
\centering
\caption{Ablation study: Clustering quality metrics. Higher silhouette scores indicate better-separated clusters in embedding space.}
...
MMM+TI+SCL (Full) & 0.575 & 0.035 & 0.572 & 0.004 \\
\bottomrule
\end{tabular}
\end{table}
...
All models converge smoothly without overfitting, with validation losses closely tracking training losses throughout the 40-epoch training period. Final validation losses range from 2.32 to 2.45, with no clear winner among the variants. The full model (MMM+TI+SCL) achieves a final validation loss of 2.36, comparable to single-objective models (MMM-only: 2.36, TI-only: 2.37, SCL-only: 2.33). The loss curves comparing all six variants are shown in Figure~\ref{fig:ablation_loss_curves}.
...
\caption{Ablation study: Training and validation loss curves for all six model variants. All models converge smoothly with similar final losses, indicating comparable optimization dynamics across objectives.}
\label{fig:ablation_loss_curves}
\end{figure}

\subsection{Analysis and Discussion}

The ablation study reveals several key findings about the role of pre-training objectives in ABC2Vec. The remarkably consistent performance across all six variants suggests that the three objectives (MMM, TI, SCL) capture overlapping aspects of musical structure rather than complementary information. Each objective alone is sufficient to learn embeddings that perform comparably on downstream tasks. This redundancy may stem from the fact that all objectives operate on the same bar-level representations and optimize for similarity relationships within the folk music domain.

Combining multiple objectives (MMM+SCL, TI+SCL, or MMM+TI+SCL) does not yield substantial improvements over single-objective models. The full model's performance is statistically indistinguishable from simpler variants on most metrics. This contrasts with findings in vision (e.g., SimCLR + supervised learning) and NLP (e.g., masked LM + next sentence prediction) where multi-task objectives often provide complementary signals. The lack of synergy in ABC2Vec suggests that folk music representation may be over-determined by the limited structural diversity in traditional tune repertoire (AABB form, 16-32 bars, modal scales).

While classification accuracies are nearly identical across variants, clustering quality metrics show modest variation, with the full model achieving slightly higher silhouette scores. This suggests that combining objectives affects the geometric structure of embedding space (how densely tunes cluster) without improving linear separability for classification. These structural differences may be relevant for similarity search, variant detection, or other retrieval-based tasks that depend on local neighborhood structure rather than global class boundaries.
