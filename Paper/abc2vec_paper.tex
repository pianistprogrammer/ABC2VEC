...
\begin{enumerate}
\item \textbf{ABC2Vec architecture}: A 6-layer Transformer encoder with bar-level patchification trained via three self-supervised objectives—Masked Music Modeling (MMM), Transposition Invariance (TI), and Section Contrastive Learning (SCL)—on 216K Irish folk tunes.
...
As shown in Figure~\ref{fig:dataset_stats}, the dataset exhibits characteristics typical of Irish traditional music. Structurally, the median tune contains 18 bars (287 characters), well within our 64-bar maximum context window, with only 3.2\% requiring truncation. Rhythmically, reels dominate at 44.9\%, followed by jigs (21.3\%), polkas (14.5\%), and waltzes (12.2\%), reflecting authentic session repertoire distributions. Tonally, major mode predominates (80.2\%), with substantial modal diversity including minor (11.3\%), Dorian (5.4\%), and Mixolydian (3.0\%), underscoring folk music's departure from classical major-minor tonality. Key roots cluster heavily in sharp keys---G (30.5\%), D (26.8\%), and A (13.9\%)---reflecting the instrumental constraints of fiddle, flute, and whistle playing, with remaining keys including C, F, E, B$\flat$, B, E$\flat$, and a small proportion (N) with unspecified or ambiguous key signatures. Meter signatures strongly correlate with tune types ($\rho = 0.87$): 4/4 for reels, 6/8 for jigs, 3/4 for waltzes. 
...
From the 214,122 training tunes, we reserve 5\% (10,469 tunes) as a \textbf{validation set} for hyperparameter tuning, training monitoring, and checkpoint selection, using stratified sampling to preserve the tune-type distribution. The remaining 95\% (198,893 tunes) forms the \textbf{training set} used exclusively for self-supervised pre-training. After deduplication across all three partitions, the final corpus totals 211,524 unique Irish traditional tunes. Table~\ref{tab:preprocessing_stats} presents key preprocessing statistics across the complete dataset. The complete processed dataset with train/validation/test splits is publicly available at \url{https://huggingface.co/datasets/pianistprogrammer/abc2vec-irish-folk-dataset}.
...
Given a corpus of folk tunes $\mathcal{D} = \{x_1, \ldots, x_N\}$ in ABC notation, our goal is to learn an encoder $f_\theta: \mathcal{X} \rightarrow \mathbb{R}^d$ that maps each tune $x_i$ to a $d$-dimensional embedding $\mathbf{z}_i = f_\theta(x_i)$ by optimizing self-supervised objectives such that musically similar tunes are represented by embeddings with high cosine similarity:
\begin{equation}
\text{sim}(x_i, x_j) = \frac{\mathbf{z}_i \cdot \mathbf{z}_j}{\|\mathbf{z}_i\| \|\mathbf{z}_j\|}.
\label{similarity}
\end{equation}

Musical similarity is multifaceted, encompassing melodic contour, rhythmic patterns (tune type), tonality (mode/key), and structural form. A well-trained embedding should capture these relationships automatically, without requiring explicit supervision. Figure~\ref{fig:architecture} provides an overview of the ABC2Vec architecture and training pipeline, showing the flow from ABC notation input through bar-level patchification, Transformer encoding, and pooling to produce dense tune embeddings, along with the three self-supervised training objectives.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{architecture_diagram.png}
\caption{ABC2Vec architecture and training pipeline. The model processes ABC notation through bar patchification, where individual bars are tokenized and embedded using character-level embeddings with positional encoding ($h_1, h_2, h_3, \ldots, h_K$). These bar representations are fed into a 6-layer Transformer encoder (d\_model=256) with stacked self-attention layers, followed by mean pooling to produce the final 128-dimensional tune embedding (d\_embed=128). The model is trained with three self-supervised objectives: Masked Music Modeling (MMM) reconstructs randomly masked bars from context, Transposition Invariance (TI) learns pitch-invariant representations by maximizing similarity between transposed versions of the same tune, and Section Contrastive Learning (SCL) distinguishes between different sections within a tune.}
\label{fig:architecture}
\end{figure}

A typical Irish jig contains 16--32 bars, yielding sequences of 200--400 characters. Direct character-level tokenization would result in impractically long sequences for Transformer processing.
...
ABC2Vec is trained via self-supervised learning using three complementary objectives:
...
\subsubsection{Section Contrastive Learning (SCL)}

Folk tunes typically follow AABB sectional form, where A and B represent distinct melodic phrases. To capture structural patterns, we apply contrastive learning at the section level. For tunes with identifiable sections (detected via repeat markers or bar grouping), we compute section-level embeddings by mean pooling bar representations within each section. Positive pairs consist of different sections from the same tune, while negative pairs consist of sections from different tunes. The SCL loss encourages the model to distinguish between sections while maintaining tune-level coherence:
\begin{equation}
\mathcal{L}_{\text{SCL}} = -\log \frac{\exp(\text{sim}(\mathbf{z}_A, \mathbf{z}_B) / \tau)}{\sum_{j \neq i} \exp(\text{sim}(\mathbf{z}_A, \mathbf{z}_{B_j}) / \tau)}
\end{equation}
where $\mathbf{z}_A$ and $\mathbf{z}_B$ are section embeddings from the same tune, and $\mathbf{z}_{B_j}$ are section embeddings from other tunes in the batch.

\subsubsection{Combined Objective}

The final training loss is a weighted combination:
\begin{equation}
\mathcal{L} = \lambda_{\text{MMM}} \mathcal{L}_{\text{MMM}} + \lambda_{\text{TI}} \mathcal{L}_{\text{TI}} + \lambda_{\text{SCL}} \mathcal{L}_{\text{SCL}}
\end{equation}
We set $\lambda_{\text{MMM}} = 1.0$, $\lambda_{\text{TI}} = 0.5$, and $\lambda_{\text{SCL}} = 0.5$ based on validation performance. The ablation study in Section~\ref{sec:ablation} analyzes the contribution of each objective.

\subsection{Implementation Details}

Our training procedure employs the AdamW optimizer with learning rate $1 \times 10^{-4}$ (1000-step linear warmup followed by cosine decay) and weight decay of 0.01, training for 40 epochs (approximately 40,000 steps) with batch size 128. Gradient clipping at norm 1.0 prevents explosion during early training when the randomly initialized decoder produces unstable reconstructions. Training is conducted on a single Apple M4 Mac with 48GB unified memory, completing in 18 hours. We save checkpoints every 2,000 steps and select the best model by validation loss. The validation loss decreases monotonically with no overfitting observed, and we use the final checkpoint at step 40,000 as our best model.
...
We evaluate ABC2Vec on multiple tasks: tune type classification, mode classification, linear probing for property disentanglement, clustering quality, and visualization. To assess whether self-supervised representation learning provides meaningful improvements over simpler approaches, we compare ABC2Vec against a Term Frequency-Inverse Document Frequency (TF-IDF) baseline that represents tunes as bags of character n-grams extracted from raw ABC notation strings.

\subsection{Training Convergence}
The total combined loss as seen in Figure~\ref{fig:training} (top left panel) exhibits smooth, monotonic convergence from an initial value of 7.77 at step 100 to a final training value of 2.48 at step 40,000, with no significant oscillations or instability. The validation loss begins at 2.66 (step 2000) and decreases steadily to 2.35 (step 40,000). Critically, the validation loss (shown in red) tracks the training loss (shown in blue) closely throughout training, with a consistent gap of approximately 0.1--0.2 loss units. This tight coupling between training and validation curves indicates that the model generalizes well to unseen tunes and is not merely memorizing the training set. The validation loss continues to improve through the entire training run, reaching its minimum of 2.35 at step 40,000, which we select as our final checkpoint. The absence of severe overfitting despite training on 211,524 tunes for 40 epochs demonstrates the effectiveness of our regularization strategy (dropout, weight decay) and the robustness of self-supervised objectives to dataset size.

The Masked Music Modeling loss (top right panel) decreases substantially from 4.78 at step 100 to 2.38 at step 40,000 for training, representing a 50\% reduction that indicates the model is learning effective bar-level representations. For validation, MMM loss starts at 2.64 (step 2000) and decreases to 2.35 (step 40,000). The loss decreases rapidly in the first 5,000 steps as the model learns basic melodic patterns (common note sequences, rhythmic motifs, cadential figures), then continues to decrease more gradually as it captures subtler musical relationships. The final MMM loss of 2.38 corresponds to reasonably accurate bar reconstruction: qualitative inspection of reconstructed bars shows that the model correctly predicts note pitches and durations for approximately 60--70\% of masked positions, with errors typically involving confusions between melodically plausible alternatives (e.g., predicting A instead of G in a descending scale passage) rather than nonsensical outputs. This level of reconstruction accuracy, while not perfect, is sufficient to learn semantically meaningful embeddings, as evidenced by downstream task performance.


\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{training_curves_hq.pdf}
\caption{Training and validation loss curves for ABC2Vec's three self-supervised objectives over 40 epochs. The model demonstrates smooth convergence without overfitting across all objectives: total combined loss (top left), Masked Music Modeling reconstruction loss (top right), Transposition Invariance contrastive loss (bottom left), and Section Contrastive Loss (bottom right). Validation loss (red) tracks training loss (blue) closely, indicating strong generalization.}
\label{fig:training}
\end{figure}
...
The Section Contrastive Loss (bottom right panel) decreases from 3.58 at step 100 to 0.19 at step 40,000, capturing section-level structural patterns in AABB-form tunes. The smooth convergence indicates that the model successfully learns to distinguish between sections within the same tune while maintaining coherent tune-level representations. The ablation study in Section~\ref{sec:ablation} examines the contribution of SCL relative to other objectives.

Overall, the training curves validate several key design decisions. First, the smooth convergence without catastrophic loss spikes confirms that our combination of MMM, TI, and SCL objectives is stable and does not suffer from conflicting gradient signals. Second, the tight train-validation gap demonstrates effective generalization despite the model's capacity (4.99M parameters) being sufficient to potentially overfit. Third, the distinct dynamics of MMM (steady decrease), TI (rapid initial drop followed by gradual convergence), and SCL (smooth monotonic decrease) suggest that these objectives capture different aspects of musical structure. The checkpoint at step 40,000, where validation loss reaches its minimum of 2.35, is selected as our final model, and all subsequent evaluation results are reported using this checkpoint.
...
\begin{table}[!htb]
\centering
\caption{Tune type classification accuracy (5-fold cross-validation on 2,078 test tunes). Random baseline represents uniform chance level (1/6 classes = 16.7\%).}
\label{tab:tune_type}
\begin{tabular}{lcccc}
\toprule
...
While direct cross-dataset comparison is inherently limited, we contextualize our results against related music representation models to situate ABC2Vec within the broader landscape. ABC2Vec achieves 78.3\% tune type accuracy on Irish folk music, which is numerically similar to MusicBERT's 76.4\% genre classification accuracy~\cite{MusicBERT2022}, though the latter was evaluated on a different corpus and task. Notably, ABC2Vec uses a considerably smaller model architecture (6 layers, 256-dim vs 12 layers, 768-dim), suggesting that the structural regularities of folk music (AABB form, limited harmony, modal scales) may require less representational capacity than the diverse repertoires targeted by general-purpose music models. CLaMP~\cite{CLaMP2023} reports 82.1\% classification accuracy on WikiMT, a classical and jazz corpus with longer, harmonically richer compositions than folk melodies; however, CLaMP focuses on cross-modal music-text retrieval rather than music-only embeddings. These comparisons should be interpreted cautiously due to differences in datasets (Irish folk vs. classical/jazz/pop), tasks (tune type vs. genre), label granularity, and musical characteristics, but they suggest that ABC2Vec's performance is reasonable for a domain-specific model operating on a structurally constrained musical tradition.


\section{Ablation Study}\label{sec:ablation}

To understand the contribution of each pre-training objective, we conduct a systematic ablation study by training five model variants with different objective combinations. All variants use identical architecture (6 layers, 256-dimensional, 8 attention heads) and training hyperparameters, differing only in which loss functions are active during training. This controlled setup allows us to isolate the contribution of each objective and assess whether objectives exhibit synergistic effects when combined. We compare these variants against our full model combining all three objectives (MMM+TI+SCL) presented in Section~\ref{sec:results}.

\subsection{Experimental Setup}

We train the following five variants:

\begin{enumerate}
    \item \textbf{MMM-only}: Masked Music Modeling alone ($\lambda_{\text{MMM}}=1.0$, others $=0$)
    \item \textbf{TI-only}: Transposition Invariance alone ($\lambda_{\text{TI}}=1.0$, others $=0$)
    \item \textbf{SCL-only}: Section Contrastive Learning alone ($\lambda_{\text{SCL}}=1.0$, others $=0$)
    \item \textbf{MMM+SCL}: Combines reconstruction and section structure ($\lambda_{\text{MMM}}=1.0$, $\lambda_{\text{SCL}}=0.5$)
    \item \textbf{TI+SCL}: Combines transposition invariance and section structure ($\lambda_{\text{TI}}=1.0$, $\lambda_{\text{SCL}}=0.5$)
\end{enumerate}

Each variant is trained for 40 epochs on the same 198,893 training tunes, using identical optimization settings (AdamW, $\text{lr}=10^{-4}$, batch size 128, warmup 1000 steps). We evaluate all variants on the test set using the same downstream tasks: tune type classification, mode classification, linear probing on multiple properties, and clustering quality metrics. This ensures fair comparison across all model configurations.

\subsection{Results}

\subsubsection{Classification Performance}

All five ablation variants achieve nearly identical performance on both tune type and mode classification tasks. Tune type accuracy ranges narrowly from 46.0\% to 46.9\% and mode accuracy remains consistently at 80.6-80.7\% across all variants. These results are statistically indistinguishable from each other and comparable to our model trained with all three objectives (46.7\% tune type, 80.7\% mode). This suggests that the pre-training objectives primarily shape the embedding space structure rather than directly improving downstream classification accuracy. The detailed results are shown in Table~\ref{tab:ablation_classification}.

\begin{table}[H]
\centering
\caption{Ablation study: Tune type and mode classification performance (5-fold CV). All ablation variants show comparable accuracy to the full model (46.7\% tune type, 80.7\% mode).}
\label{tab:ablation_classification}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \multicolumn{2}{c}{\textbf{Tune Type}} & \multicolumn{2}{c}{\textbf{Mode}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
 & Accuracy & Std Dev & Accuracy & Std Dev \\
\midrule
MMM-only & 0.466 & 0.010 & 0.807 & 0.001 \\
TI-only & 0.462 & 0.007 & 0.806 & 0.002 \\
SCL-only & 0.460 & 0.009 & 0.807 & 0.001 \\
MMM+SCL & 0.469 & 0.008 & 0.807 & 0.001 \\
TI+SCL & 0.463 & 0.008 & 0.807 & 0.001 \\
\bottomrule
\end{tabular}
\end{table}

The remarkably consistent mode classification accuracy (80.7\% $\pm$ 0.001) across all variants indicates that mode information is preserved regardless of the pre-training objective. This likely reflects the fact that mode is strongly correlated with surface-level statistical patterns (scale degree distributions, cadence patterns) that are captured even by single-objective models. The modest tune type performance across all variants (46-47\% vs. 16.7\% chance baseline) suggests that rhythm recognition from symbolic notation is inherently challenging, requiring nuanced understanding of temporal patterns that none of the objectives fully capture.

\subsubsection{Linear Probing and Multi-Task Learning}

Linear probing uses a single-layer ridge classifier trained on frozen embeddings, providing a stricter test of embedding quality than full fine-tuning. Across four properties—tune type, mode, key root, and tune length—all ablation variants perform comparably to the three-objective model (46.8\% tune type, 80.7\% mode, 31.4\% key root, 93.4\% tune length), with tune type at 46.1-46.8\%, mode at 80.6-80.7\%, key root at 31.7-32.2\%, and tune length at 93.2-93.8\%. These results mirror the classification findings and are detailed in Table~\ref{tab:ablation_linear_probing}.

\begin{table}[H]
\centering
\caption{Ablation study: Linear probing accuracy on multiple properties. All ablation variants perform comparably to the full model.}
\label{tab:ablation_linear_probing}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Tune Type} & \textbf{Mode} & \textbf{Key Root} & \textbf{Tune Length} \\
\midrule
MMM-only & 0.463 & 0.807 & 0.322 & 0.934 \\
TI-only & 0.462 & 0.806 & 0.320 & 0.933 \\
SCL-only & 0.461 & 0.807 & 0.317 & 0.933 \\
MMM+SCL & 0.468 & 0.807 & 0.317 & 0.938 \\
TI+SCL & 0.462 & 0.807 & 0.319 & 0.932 \\
\bottomrule
\end{tabular}
\end{table}

The high tune length accuracy (93+\%) across all variants indicates that sequence length information is strongly preserved in embeddings regardless of pre-training objective. Key root performance remains modest (31-32\%), consistent with earlier findings that pitch encoding is only partially removed despite the Transposition Invariance objective. Notably, TI-only does not significantly outperform other variants on key root classification, suggesting that transposition invariance alone is insufficient to fully disentangle pitch from other musical properties.

\subsubsection{Clustering Quality}

Unsupervised clustering metrics reveal that all ablation variants produce embeddings with some internal structure, though clusters are not sharply separated. Silhouette scores range from 0.491 to 0.566, with SCL-only achieving 0.566 and our combined-objective model achieving 0.575, suggesting that training with all three objectives may yield slightly more structured embeddings, though the difference is modest. These results are summarized in Table~\ref{tab:ablation_clustering}.

\begin{table}[H]
\centering
\caption{Ablation study: Clustering quality metrics. The full model (0.575 silhouette) shows modest improvement over ablation variants.}
\label{tab:ablation_clustering}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \multicolumn{2}{c}{\textbf{Tune Type}} & \multicolumn{2}{c}{\textbf{Mode}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
 & Silhouette & NMI & Silhouette & NMI \\
\midrule
MMM-only & 0.491 & 0.036 & 0.571 & 0.004 \\
TI-only & 0.553 & 0.032 & 0.554 & 0.005 \\
SCL-only & 0.566 & 0.038 & 0.565 & 0.005 \\
MMM+SCL & 0.536 & 0.041 & 0.553 & 0.006 \\
TI+SCL & 0.549 & 0.038 & 0.551 & 0.005 \\
\bottomrule
\end{tabular}
\end{table}

Normalized Mutual Information (NMI) scores remain low across all variants (0.032-0.041 for tune type, 0.004-0.006 for mode), indicating weak alignment between K-means clusters and ground-truth labels. This confirms that embeddings do not form discrete, label-aligned clusters, consistent with our earlier finding that folk music similarity is continuous rather than categorical. The modest silhouette scores combined with low NMI suggest that embeddings capture gradual variation and family resemblance rather than crisp category boundaries.

\subsubsection{Training Convergence}

All ablation variants converge smoothly without overfitting, with validation losses closely tracking training losses throughout the 40-epoch training period. Final validation losses range from 2.32 to 2.45 across the five ablation variants, with our three-objective model achieving 2.35, comparable to the ablation models (MMM-only: 2.36, TI-only: 2.37, SCL-only: 2.33). The loss curves comparing all variants are shown in Figure~\ref{fig:ablation_loss_curves}.

Convergence speed is also similar across variants, with all models reaching a validation loss below 2.5 by approximately 10,000-15,000 steps. This indicates that no single objective provides a substantial training efficiency advantage. The smooth convergence and lack of overfitting across all configurations suggest that the model capacity (6 layers, 256-dimensional) is well-matched to the task complexity, and that all objectives provide sufficient learning signal for effective representation learning.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{ablation_loss_curves.pdf}
\caption{Ablation study: Training and validation loss curves for five ablation variants compared to the full model. All models converge smoothly with similar final losses, indicating comparable optimization dynamics across objectives.}
\label{fig:ablation_loss_curves}
\end{figure}

\subsection{Analysis and Discussion}

The ablation study reveals several key findings about the role of pre-training objectives in ABC2Vec. The remarkably consistent performance across all five ablation variants and our complete model suggests that the three objectives (MMM, TI, SCL) capture overlapping aspects of musical structure rather than complementary information. Each objective alone is sufficient to learn embeddings that perform comparably on downstream tasks. This redundancy may stem from the fact that all objectives operate on the same bar-level representations and optimize for similarity relationships within the folk music domain.

Combining multiple objectives (MMM+SCL, TI+SCL) does not yield substantial improvements over single-objective models. Even our model trained with all three objectives performs statistically indistinguishably from simpler variants on most metrics. This contrasts with findings in vision (e.g., SimCLR + supervised learning) and NLP (e.g., masked LM + next sentence prediction) where multi-task objectives often provide complementary signals. The lack of synergy in ABC2Vec suggests that folk music representation may be over-determined by the limited structural diversity in traditional tune repertoire (AABB form, 16-32 bars, modal scales).

While classification accuracies are nearly identical across all variants, clustering quality metrics show modest variation, with the combined-objective model achieving slightly higher silhouette scores (0.575) compared to the best ablation variant (SCL-only: 0.566). This suggests that combining objectives affects the geometric structure of embedding space (how densely tunes cluster) without improving linear separability for classification. These structural differences may be relevant for similarity search, variant detection, or other retrieval-based tasks that depend on local neighborhood structure rather than global class boundaries.

Given the comparable performance of single-objective models, practitioners may opt for simpler training pipelines (e.g., MMM-only) to reduce computational cost and hyperparameter tuning. However, the model trained with all three objectives shows slightly superior clustering quality and balanced loss convergence, suggesting that multi-objective training may still be preferable for applications requiring well-structured embedding spaces, such as visualization, exploratory search, or hierarchical clustering.

The ablation study was conducted on 198,893 training tunes, representing a large-scale folk music corpus. It is possible that objective differences become more pronounced with smaller datasets, where each objective's inductive bias plays a stronger role. However, at the scale of our dataset, all three objectives converge to similar representations.

In summary, the ablation study demonstrates that ABC2Vec's strong performance is not critically dependent on any single pre-training objective. All three objectives (MMM, TI, SCL) individually capture sufficient musical structure to yield effective embeddings, and combining them provides marginal improvements in clustering quality without enhancing classification accuracy. These findings suggest that the model's success stems from the overall architecture (bar-level Transformer with contrastive and reconstructive learning) rather than a specific objective design, and that simpler training configurations may suffice for many practical applications.
