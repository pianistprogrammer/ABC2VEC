...
All five ablation variants achieve nearly identical performance on both tune type and mode classification tasks. Tune type accuracy ranges narrowly from 46.0\% to 46.9\% and mode accuracy remains consistently at 80.6-80.7\% across all variants. These results are statistically indistinguishable from each other and comparable to the full model (MMM+TI+SCL: 46.7\% tune type, 80.7\% mode). This suggests that the pre-training objectives primarily shape the embedding space structure rather than directly improving downstream classification accuracy. The detailed results are shown in Table~\ref{tab:ablation_classification}.
...
Unsupervised clustering metrics reveal that all ablation variants produce embeddings with some internal structure, though clusters are not sharply separated. Silhouette scores range from 0.491 to 0.566, with SCL-only achieving 0.566 and the full model (MMM+TI+SCL) achieving 0.575, suggesting that combining objectives may yield slightly more structured embeddings, though the difference is modest. These results are summarized in Table~\ref{tab:ablation_clustering}.
...
The ablation study reveals several key findings about the role of pre-training objectives in ABC2Vec. The remarkably consistent performance across all five ablation variants and the full model suggests that the three objectives (MMM, TI, SCL) capture overlapping aspects of musical structure rather than complementary information. Each objective alone is sufficient to learn embeddings that perform comparably on downstream tasks. This redundancy may stem from the fact that all objectives operate on the same bar-level representations and optimize for similarity relationships within the folk music domain.

Combining multiple objectives (MMM+SCL, TI+SCL) does not yield substantial improvements over single-objective models. Even the full model (MMM+TI+SCL) performs statistically indistinguishably from simpler variants on most metrics. This contrasts with findings in vision (e.g., SimCLR + supervised learning) and NLP (e.g., masked LM + next sentence prediction) where multi-task objectives often provide complementary signals. The lack of synergy in ABC2Vec suggests that folk music representation may be over-determined by the limited structural diversity in traditional tune repertoire (AABB form, 16-32 bars, modal scales).

While classification accuracies are nearly identical across all variants, clustering quality metrics show modest variation, with the full model achieving slightly higher silhouette scores (0.575) compared to the best ablation variant (SCL-only: 0.566). This suggests that combining objectives affects the geometric structure of embedding space (how densely tunes cluster) without improving linear separability for classification. These structural differences may be relevant for similarity search, variant detection, or other retrieval-based tasks that depend on local neighborhood structure rather than global class boundaries.

Given the comparable performance of single-objective models, practitioners may opt for simpler training pipelines (e.g., MMM-only) to reduce computational cost and hyperparameter tuning. However, the full model's slightly superior clustering quality and balanced loss convergence suggest that multi-objective training may still be preferable for applications requiring well-structured embedding spaces, such as visualization, exploratory search, or hierarchical clustering.
