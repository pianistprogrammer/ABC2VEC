...
\text{sim}(x_i, x_j) = \frac{\mathbf{z}_i \cdot \mathbf{z}_j}{\|\mathbf{z}_i\| \|\mathbf{z}_j\|}
\end{equation}
where $\mathbf{z}_i = f_\theta(x_i)$ and $\mathbf{z}_j = f_\theta(x_j)$ are the normalized 128-dimensional embeddings.
...
To investigate whether different properties utilize disjoint or overlapping embedding dimensions, we compute feature importance scores by analyzing the magnitude of learned linear weights for each property. Figure~\ref{fig:dimension_importance} visualizes the top-k most important dimensions for each property. Tune type relies most heavily on dimensions $\{12, 27, 33, 34, 41\}$, while mode utilizes dimensions $\{1, 9, 15, 28, 45\}$. Strikingly, these dimension sets exhibit minimal overlap, with only dimension 80 shared across both properties. This pattern of semi-independent subspaces suggests \textit{distributed disentanglement}: different musical properties are encoded in largely non-overlapping regions of the 128-dimensional space, enabling independent manipulation or probing of specific attributes. However, the disentanglement is not complete---some dimensions contribute weakly to multiple properties, and the nonlinear mode encoding indicates cross-dimensional interactions. The partial disentanglement likely emerges naturally from the self-supervised objectives (MMM, TI), which encourage the model to allocate representational capacity to orthogonal aspects of musical structure (rhythm, tonality, pitch-class) without explicit disentanglement constraints.
...
We apply UMAP~\cite{UMAP2018} and t-SNE~\cite{tSNE2008} for 2D visualization of the 128-dimensional embedding space. Figure~\ref{fig:umap_combined}(a) colors points by tune type, revealing partial visual clustering (reels form a cohesive region, jigs scatter more). Figure~\ref{fig:umap_combined}(b) colors by mode, showing major/minor separation but mixolydian/dorian overlap. These visualizations corroborate quantitative findings: embeddings capture musical structure but do not form hard boundaries.

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{umap_combined.png}
\caption{UMAP projection of ABC2Vec embeddings. (a) Colored by tune type: reels (blue, 44.9\% of dataset) form a cohesive central cluster, reflecting their consistent 4/4 meter and straight eighth-note rhythm. Jigs (green, 21.3\%) form a distinct cluster, separated from reels by their characteristic 6/8 meter and lilting feel. Polkas (orange, 16.0\%) and waltzes (red, 11.4\%) occupy separate regions, demonstrating that the model captures their distinctive rhythmic signatures (2/4 and 3/4 respectively). Slides and slip jigs, being smaller classes, show more scattered distributions. The clear separation between major rhythmic categories (reels vs jigs vs waltzes) validates that ABC2Vec learns meaningful representations of folk music structure, though the partial overlap between related categories reflects genuine rhythmic ambiguity in the 2/4 family of dance tunes. (b) Colored by mode: major (red, 80.7\% of dataset) dominates with multiple distinct clusters, reflecting the diversity of melodic contours within major-mode tunes. Minor (blue, 11.0\%) forms well-separated regions, demonstrating that the model effectively distinguishes major from minor tonality despite the two modes sharing similar rhythmic patterns. Dorian (green, 5.4\%) and Mixolydian (orange, 2.9\%) exhibit more distributed patterns with some overlap with major clusters, consistent with these modal scales' intermediate tonal character---Dorian shares major's brightness but with minor's sixth degree, while Mixolydian shares major's scale structure but with a flatted seventh. The visualization corroborates our quantitative finding of 80.5\% mode classification accuracy: major and minor are strongly separated (high precision), while modal categories exhibit more ambiguity due to their tonal proximity to major keys.}
